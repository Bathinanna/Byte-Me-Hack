  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col max-w-[70%] ${isOwn ? "items-end" : "items-start"}`}>
        ...
        <div className="relative">
          <div
            className={`px-4 py-2 rounded-2xl max-w-md ${
              isOwn ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-100 border border-slate-700"
            } ${message.pinned ? "ring-2 ring-yellow-500/50" : ""}`}
          >
            {/* Pin indicator */}
            {message.pinned && <Pin className="absolute -top-1 -right-1 w-4 h-4 text-yellow-500 fill-current" />}
            ...
            {message.content.startsWith("/uploads/") && (
              <div className="mt-2">
                {message.content.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                  ...
                  <a href={message.content} className="flex items-center gap-2 text-blue-400 hover:text-blue-300">
            ...
            {message.reactions.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {Object.entries(
                  ...
                  <button
                    key={emoji}
                    className="flex items-center gap-1 px-2 py-1 bg-slate-700/50 rounded-full text-xs hover:bg-slate-700"
          ...
          {/* Quick actions */}
          {showActions && (
            <div className={`absolute top-0 flex items-center gap-1 ${isOwn ? "-left-20" : "-right-20"}`}>
              <button
              ...
              <button
                ...
                className="p-1.5 bg-slate-800 border border-slate-700 rounded-lg hover:bg-slate-700 transition-colors"
              >
                <Pin className={`w-4 h-4 ${message.pinned ? "text-yellow-500" : "text-slate-400"}`} />
              </button>
              {isOwn && (
                <button
                  onClick={() => onDelete(message.id)}
                  className="p-1.5 bg-slate-800 border border-slate-700 rounded-lg hover:bg-red-600 transition-colors"
                >
          ...
          {showReactions && (
            <div
              className={`absolute top-8 z-10 flex gap-1 p-2 bg-slate-800 border border-slate-700 rounded-lg shadow-lg ${isOwn ? "-left-24" : "-right-24"}`}
            >
        ...
        {isOwn && (
          <div className="text-xs text-slate-500 mt-1 px-1">{format(new Date(message.createdAt), "HH:mm")}</div>
        )}
      </div>
    </div>
  )
